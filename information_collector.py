...
def fetch_patent_data(query, dir_path):
    ...
    if response.status_code == 200:
        ...
        for idx, patent in enumerate(data.get("organic_results", [])):
            ...
            patent_citations = response_data.get("patent_citations", {}).get("original", {})

            for idx2, citation in enumerate(patent_citations):
                if idx2 >= 10:
                    print(f"Reached 10 citations for patent {idx}. Moving to next patent.")
                    break
                ...
                if serpapi_url2:
                    ...
                    if citation_data:
                        ...
                        print(f"Error fetching data for citation {idx2}: No data returned.")
                else:
